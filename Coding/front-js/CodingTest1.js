...
function listUndoneTasks02(items) {
  ...
  items.filter((item) => {
    return item.done === false && arr.push(item)
  })
  return arr.map((item) => {
    return formatTask01(item)
  })
}
...
function addUniqueTask15(newTask, items) {
  if (!items.some((item) => item.title === newTask.title)) items.push(newTask)
}
...
const todos19 = [{ name: "submit", done: false }]
...
function groupByDone20(items) {
  return {
    done: items.filter((item) => item.done === true),
    false: items.filter((item) => item.done === false),
  }
}
